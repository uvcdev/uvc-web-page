<?php
    class AdminPartnerModel extends gf{
        private $json;
        private $dir;
        private $conn;
        ...
        *********************************************************************/
        function partner_insert() {
            ...
            if($this->value_check(array("image_sync"))){ //필수값 체크하기
                ...
                // 해당 폴더가 없으면 생성
                if(!is_dir($this->dir."_upload/partner")){
                    umask(0);
                    if(!mkdir($this->dir."_upload/partner", 0777, true)){
                        return;
                    }
                }
                ...
                for($i = 1; $i <= $param["langLength"]; $i++) {
                    ...
                    if($_FILES["img_file_".$lang_idx]["error"][0] != 4) {
                        ...
                        while(file_exists($this->dir."_upload/partner/".$rename)){

                            $rename = md5(uniqid($tempFile)) .round(microtime(true)).'.'.$fileTypeExt[1];
                        }
                        $fileSave = $this->dir."_upload/partner/".$rename;
                ...
                $key_code = $this->key_code_check("partner");

                $query = "INSERT INTO partner(real_img, upload_img, image_sync, lang_idx, key_code, reg_date) VALUES(";
                for($i = 1; $i <= $param["langLength"]; $i++) {
                ...
                if($result["error_code"] != null) {
                    $query = "DELETE FROM partner WHERE idx = ".$result["value"];
        ...
        *********************************************************************/
        function partner_select() {
            ...
            if($this->value_check(array("page_size", "move_page"))){
                ...
                $query = "SELECT * FROM partner WHERE lang_idx = ".$param["lang_idx"]." ORDER BY seq ASC, reg_date DESC LIMIT ".$page_size * ($move_page - 1).", ".$page_size;
                $result = $this->conn->db_select($query);

                if($result["result"] == "1") {
                    $this->result = $result;
                    $query = "SELECT COUNT(idx) AS total_count FROM partner WHERE lang_idx = ".$param["lang_idx"];
        ...
        *********************************************************************/
        function partner_delete() {
            ...
            $query = "SELECT upload_img FROM partner WHERE key_code = ";
            ...
            if($result["result"] == "1") {
                ...
                $query = "DELETE FROM partner WHERE key_code = ";
                for($i = 0; $i < count($key_code); $i++) {
                    if($i == 0) {
                        $query = $query.$this->null_check($key_code[$i]);
                    }
                ...
                if($result["result"] == "1") {
                    if($image_unique != null) {
                        for($j = 0; $j < count($image_unique); $j++) {
                            ...
                            unlink($this->project_name."/_upload/partner/".$image_unique[$j]);
        ...
        *********************************************************************/
        function modify_detail() {
            ...
            $query = "SELECT * FROM partner WHERE key_code = ".$this->null_check($param["key_code"]);
            $result = $this->conn->db_select($query);
        ...
        *********************************************************************/
        function modify_update() {
            ...
            if($this->value_check(array("key_code"))){
                $query = "SELECT upload_img, real_img, image_sync, lang_idx FROM partner WHERE key_code = ".$this->null_check($param["key_code"]);
                $result = $this->conn->db_select($query);
                ...
                if($result["result"] == "1") {
                    ...
                    if(count($del_sync_image) > 0) {
                        for($i = 0; $i < count($del_sync_image); $i++) {
                            unlink($this->project_name."/_upload/partner/".$del_sync_image[$i]);
                        }
                    }
                        ...
                        for($j = 1; $j <= $param["langLength"]; $j++) {
                            for($i = 0; $i < count($diff_image); $i++) {
                                if($diff_image[$i] == $delImg){
                                    ...
                                    $query = "UPDATE partner SET upload_img = null, real_img = null WHERE real_img = ".$this->null_check($delImg);
                    ...
                    //     unlink($this->project_name."/_upload/partner/".$delImg[$i]);
                    // }
        ...
        function change_seq(){
            ...
            if($this->value_check(array("idx_array"))){
                ...
                for($i = 0; $i < count($relation_array); $i++){
                    $sql = "update partner set seq = ".($i + 1);
