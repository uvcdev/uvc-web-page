<?php
    class BoardModel extends gf{
        ...
        *********************************************************************/
        ...
        /********************************************************************* 
        // 함 수 : main board Select
        // 만든이: 박여진
        // 담당자: 
        *********************************************************************/
        function main_board_select() {
            $param = $this->json;

            $query = "SELECT * FROM board where lang_idx = ".$param["lang_idx"]." LIMIT 0, 3";
        ...
        *********************************************************************/
        ...
        function board_select1() {
            $param = $this->json;
            // if($this->value_check(array("page_size", "move_page"))){
            
              
                $query = "SELECT * FROM board WHERE lang_idx = ".$param["lang_idx"]." ORDER BY date asc";
                $result = $this->conn->db_select($query);
        ...
        *********************************************************************/
        function detail_board() {
            ...
            $query = "SELECT * FROM board WHERE idx = ".$this->null_check($param["idx"])." and lang_idx = ".$param["lang_idx"];
            $result = $this->conn->db_select($query);
            
            if($result["result"] == "1"){
                $this->result = $result;
                $query = "SELECT * FROM board_file WHERE key_code = ".$this->null_check($result["value"][0]["key_code"])." and lang_idx = ".$param["lang_idx"];
        ...
        *********************************************************************/
        function neighbor_post() {
            ...
            $query = "SELECT idx, title FROM board ";
