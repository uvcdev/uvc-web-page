<?php
    class AdminNewsModel extends gf{
        private $json;
        private $dir;
        private $conn;
        ...
        *********************************************************************/
        function news_insert() {
            ...
            if($this->value_check(array("lang_idx"))){ //필수값 체크하기
                ...
                if(!is_dir($this->dir."_upload/news/file/")){
                    umask(0);
                    if(!mkdir($this->dir."_upload/news/file/", 0777, true)){
                        return;
                    }
                }

                if(!is_dir($this->dir."_upload/news/image/")){
                    umask(0);
                    if(!mkdir($this->dir."_upload/news/image/", 0777, true)){
                        return;
                    }
                }
                ...
                if($_FILES["img_file_1"]["name"][0] != null) {
                    ...
                    for($i = 0; $i < $imageCount; $i++) {
                        if($_FILES["img_file_1"]["error"][$i] != 4) {
                            ...
                            while(file_exists($this->dir."_upload/news/image/".$rename)){
    
                                $rename = md5(uniqid($tempFile[$i])) .round(microtime(true)).'.'.$fileTypeExt[1];
                            }
                            $imageSave = $this->dir."_upload/news/image/".$rename;
                ...
                if($_FILES["file_1"]["name"][0] != null) {
                    ...
                    for($j = 0; $j < $fileKor; $j++) {
                        if($_FILES["file_1"]["error"][$j] != 4) {
                            ...
                            while(file_exists($this->dir."_upload/news/file/".$rename)){
    
                                $rename = md5(uniqid($korFile[$j])) .round(microtime(true)).'.'.$fileTypeExt[1];
                            }
                            $fileSave = $this->dir."_upload/news/file/".$rename;
                ...
                $key_code = $this->key_code_check("news");

                $query = "INSERT INTO news(title, content, date, file_sync, image_sync, real_img, upload_img, lang_idx, key_code, reg_date) VALUES(";
                ...
                if($result["result"] == "1") {
                    if($file_idxs != null) {
                        $insertFlag = false;
                        $query = "INSERT INTO news_file(key_code, real_file, upload_file, lang_idx) VALUES(";
                        for($i = 1; $i <= $param["langLength"]; $i++) {
                ...
                if($result["error_code"] != null) {
                    $query = "DELETE FROM news WHERE key_code = ".$key_code;
                    $result = $this->conn->db_delete($query);

                    $query = "DELETE FROM news_file WHERE key_code = ".$key_code;
                    $result = $this->conn->db_delete($query);
                }
                else {
        ...
        *********************************************************************/
        function news_select() {
            ...
            if($this->value_check(array("page_size", "move_page"))){
                ...
                $query = "SELECT * FROM news WHERE lang_idx = ".$param["lang_idx"]." ORDER BY date DESC LIMIT ".$page_size * ($move_page - 1).", ".$page_size;
                $result = $this->conn->db_select($query);

                if($result["result"] == "1") {
                    $this->result = $result;
                    $query = "SELECT COUNT(idx) AS total_count FROM news WHERE lang_idx = ".$param["lang_idx"];
        ...
        *********************************************************************/
        function news_delete() {
            ...
            $query = "SELECT upload_img, image_sync, file_sync FROM news WHERE key_code = ";
            ...
            if($checkResult["result"] == "1") {
                ...
                $query = "SELECT upload_file FROM news_file WHERE key_code = ";
                for($i = 0; $i < count($key_code); $i++) {
                    if($i == 0) {
                        $query = $query.$this->null_check($key_code[$i]);
                    }
                ...
                if($result["result"] == "1") {
                    ...
                    $query = "DELETE FROM news WHERE key_code = ";
                    for($i = 0; $i < count($key_code); $i++) {
                        if($i == 0) {
                            $query = $query.$this->null_check($key_code[$i]);
                        }
                    ...
                    $query = "DELETE FROM news_file WHERE key_code = ";
                    for($i = 0; $i < count($key_code); $i++) {
                        if($i == 0) {
                            $query = $query.$this->null_check($key_code[$i]);
                        }
                    ...
                    if($result["result"] == "1") {
                        ...
                        if($image_unique != null) {
                            for($j = 0; $j < count($image_unique); $j++) {
                                ...
                                unlink($this->project_name."/_upload/news/image/".$image_unique[$j]);
                        ...
                        if($file_unique != null) {
                            for($j = 0; $j < count($file_unique); $j++) {                               
                                unlink($this->project_name."/_upload/news/file/".$file_unique[$j]);
                            }
                        }
                    }
                }
        ...
        *********************************************************************/
        function news_detail() {
            ...
            $query = "SELECT * FROM news WHERE key_code = ".$this->null_check($param["key_code"]);
            $result = $this->conn->db_select($query);

            if($result["result"] == "1") {
                $this->result = $result;

                $query = "SELECT * FROM news_file WHERE key_code = ".$this->null_check($param["key_code"]);
        ...
        function modify_update() {
            ...
            if($this->value_check(array("key_code"))){
                ...
                if($result["result"] == "1") {
                    ...
                    //저장된 싱크랑 넘어온 싱크가 다를때
                    //첫번째 언어를 제외한 나머지 언어의 news_file 데이터 제거
                    if($result["value"][0]["file_sync"] != $param["file_sync"]) {
                        $query = "DELETE FROM news_file WHERE key_code = ".$this->null_check($param["key_code"])." AND lang_idx != 1";
                    ...
                    if(count($del_sync_image) > 0) {
                        for($i = 0; $i < count($del_sync_image); $i++) {
                            unlink($this->project_name."/_upload/news/image/".$del_sync_image[$i]);
                        }
                        $query = "update news set upload_img = ".$this->null_check($result["value"][0]["upload_img"]).", real_img = ".$this->null_check($result["value"][0]["real_img"])." where key_code = ".$this->null_check($param["key_code"])." and lang_idx != '1'";
                    ...
                    if($delImg != null) {
                        ...
                        // for($j = 1; $j <= $param["langLength"]; $j++) {
                            for($i = 0; $i < count($diff_image); $i++) {
                                if($diff_image[$i] == $delImg[$i]){
                                    
                                    $query = "UPDATE news SET upload_img = null, real_img = null WHERE upload_img = ".$this->null_check($delImg[$i])." and key_code = ".$this->null_check($param["key_code"]);
        
                                    $this->conn->db_update($query);
                                }
                                // unlink($this->project_name."/_upload/news/image".$delImg[$i]);
                    ...
                    // if($delImg != null) {
                        ...
                        for($i = 1; $i <= $param["langLength"]; $i++) {
                            ...
                            if($_FILES["img_file_".$lang_idx]["error"][0] != 4) {
                                ...
                                $imgSave = $this->dir."_upload/news/image/".$rename;
                    ...
                    if($deleteCheck != null) {  // db에서 해당 file 삭제 서버에도 삭제
                        for($i = 0; $i < count($deleteCheck); $i++) {
                            $query = "DELETE FROM news_file WHERE upload_file = ".$this->null_check($deleteCheck[$i])." AND key_code = ".$this->null_check($param["key_code"]);
                            $result = $this->conn->db_delete($query);
                            $this->result = $result;
                        }
                    }
