...
let File = [];      //  upload_file name
let FileName = [];   // real_file name
var delFile = [];
var delImg = [];
var deleteCheck = [];
var Img = [];
						...
						for(var i = 0; i < li.length; i++) {
							...
							(function(){
								...
								return li[cur_i].onclick = function() {
                                    ...
                                    // langIdx = JSON.parse(this.dataset.value).idx;
								}
							})();
						}
					}
	...
	lb.auto_view({
		...
		havior : function(elem, data, name, copy_elem) {
			...
			if(name == "title") {
                ...
                elem.setAttribute('data-wrap', 'file_input_wrap_' + data["idx"]);
            }
        }, end : function() {
			// file_index++;
...
function init_view(result) {
    ...
    for(var i = 0; i < data.length; i++) {
        ...
        if(data[i].file_sync == 1) {
            obj.elem.sync_file.checked = true;
            obj.elem.unsync_file.checked = false;
            obj.value.file_sync = 1;
        }
        else {
            obj.elem.unsync_file.checked = true;
            obj.elem.sync_file.checked = false;
            obj.value.file_sync = 0;
        }
        document.querySelector('#title_' + data[i].lang_idx).value = data[i].title;
        ...
        if(data[i].upload_img != null) {
            image_wrap.value = data[i].upload_img;
            var img = clone.querySelector('img');
            img.setAttribute('src', upload_path + "board/image/" + data[i].upload_img);
            img.setAttribute('data-img', data[i].upload_img);
            // img.setAttribute('img_file_' + data[i].lang_idx, '['+ data[i].real_img+']');
            image_wrap.appendChild(clone);
    
            if(clone.querySelector('img').getAttribute('data-img') != null) {
                Img.push(clone.querySelector('img').getAttribute('data-img'));
            }
        }
        ...
        if(file != undefined){
            for(let j = 0; j < file.length; j++) {
                let file_wrap = document.querySelector('#file_wrap_' + data[i].lang_idx);
                let file_name = file_wrap.querySelector('#file_name_' + data[i].lang_idx + '_' + file_index);
               
                if(file[j].lang_idx == data[i].lang_idx) {
                    file_name.value = file[j].real_file;
                    
                }
                file_name.setAttribute('data-file', file[j].upload_file);
                file_name.setAttribute('file-name', file[j].real_file);
                file_wrap.querySelector('#fileDel_' + data[i].lang_idx).setAttribute('onclick', 'delete_file(this,' + data[i].lang_idx + ', ' + file_index + ');');
    
                let input = document.createElement('input');
                input.setAttribute('type', 'file');
                input.id = "file_" + data[i].lang_idx + "_" + file_index;
                input.setAttribute('name', 'file_' + data[i].lang_idx + '[]');
                input.setAttribute('onchange', 'add_file(this, ' + data[i].lang_idx + '_' + file_index);
                let file_label = document.querySelector('#file_label_' + data[i].lang_idx);
                file_label.setAttribute('for', 'file_' + data[i].lang_idx + '_' + file_index);
    
                if(file_name.getAttribute('data-file') != null) {
                    File.push(file_name.getAttribute('data-file'));
                }
    
                if(file_name.getAttribute('file-name') != null) {
                    FileName.push(file_name.getAttribute('file-name'));
                }
            }
        }
        ...
        var id = "sumnote_" + (i + 1);
        var sum_obj = lb.getElem(id).nextSibling.children[2].children[2];
        sum_obj.innerHTML = data[i].content;
        sum_note_value[id] = sum_obj;
    }
}
...
function add_image_file(event, lang_idx) {
    ...
    if(copy.querySelector('img').getAttribute('data-img') != null) {
        Img.push(copy.querySelector('img').getAttribute('data-img'));
    }
}
...
function add_file(test, lang_idx, index) {
    ...
    if(filename.getAttribute('data-file') != null) {
        deleteCheck.push(filename.getAttribute('data-file'));
    }
}
...
function delete_file(test, lang_idx, index) {
    ...
    var div_wrap = wrap.querySelector('#file_copy_' + index);
    ...
    if(wrap_div.getAttribute('data-file') != null) {
        var deleteFile = wrap_div.getAttribute('data-file');
