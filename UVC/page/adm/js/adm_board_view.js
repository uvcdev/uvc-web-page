				...
				lb.auto_view({
					...
					havior : function(elem, data, name, copy_elem){ 
						...
						if(typeof data.lang_idx != "undefined"){
							board_view(result.value[data.lang_idx - 1]);
						}else{
							board_view(result.value[0]);
						}
						...
						for(var i = 0; i<li.length; i++){
							...
							(function(){
								...
								return li[cur_i].onclick = function() {
									...
									board_view(JSON.parse(this.dataset.value));
								}
							})();
						}
					}
				...
				lb.auto_view({
					...
					havior : function(elem, data, name, copy_elem){ 
						...
						if(name == "check") {
							...
							elem.src = upload_path + "board/image/" + data['upload_img'];
							}
						}
						else if(name == "title"){
