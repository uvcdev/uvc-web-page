    ...
    lb.auto_view({
		...
		havior : function(elem, data, name, copy_elem) {
			...
			if(name == "title") {
                ...
                elem.id = "content_" + data["idx"];
				elem.setAttribute('name', 'content_' + data["idx"]);
            }
...
function init_view(){
    ...
    for(var i = 0; i < data.length; i++) {
        ...
        document.querySelector('#content_' + data[i].lang_idx).value = data[i].content;
        document.querySelector('#category').value = data[i].category;
        $('.select').selectric();
        ...
        if(data[i].upload_img != null) {
            ...
            img.setAttribute('src', upload_path + "certify/" + data[i].upload_img);
...
function register_board() {
    ...
    lb.ajax({
        ...
        list : {
            ...
            category : obj.elem.category.value,
        },
